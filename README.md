...
#### Registry
The mesh registry saves meshes generated by code to a unique `identifier` - a string, similarly to component types.

If you set `fromRegistry` to `true`, the value of `mesh` will be used to pull from the registry instead of being treated as a file path.

#### Procedural Meshes
Procedural meshes can *only* be called from the registry, and custom ones must be registered manually (example below).
...
fun main() = object : KiloEngineGame() {
    ...
    override fun loadInitScene() {
        MeshRegistry.register("identifier", MyProceduralMesh())
